...
## Prompt 04： 支援cli參數，並支援使用nuitka套件打包

```text

# 角色
你是一個資深的Python應用工程師，正準備用nuitka套件打包目前的專案應用程式。

# 任務說明
目前的專案main.py是cli應用入口，gui.py是gui應用入口，已經安裝了nuitka套件，請確認一下如何正確打包cli跟gui的exe用程式。
同時專案依賴playwright的chromium瀏覽器，但已經有下載到ms-playwright目錄裡，只是打包的時候是不是應該要順便包進去，必須要進一步判斷。 
cli模式要支援參數控制，所以main.py應該需要跟著調整，但參數應是可選的，不加參數執行的話則使用目前程式的配置。 

# 其他注意事項
- 專案有用uv進行管理
- 打包指令也請加上--lto=yes。
- 請不要在打包階段就處理應用程式簽署，會在之後再透過signtool來作業。
- 請注意程式裡面是否有用到相對路徑，確認打包後會不會受到影響。
- 完成任務後請將打包指令與打包輸出結構以及注意事項寫入README.md
